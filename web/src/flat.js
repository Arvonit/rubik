...
FlatCube.prototype.getStickers = function (top, front, right) {
  ...
  if (numFaces == 3) {
    ...
    //center
    if (top == 1) colors[TOP] = getColor(2, 4);
    else if (top == -1) colors[BOTTOM] = getColor(5, 4);
    else if (front == 1) colors[FRONT] = getColor(4, 4);
    else if (front == -1) colors[BACK] = getColor(0, 4);
    else if (right == 1) colors[RIGHT] = getColor(3, 4);
    else if (right == -1) colors[LEFT] = getColor(1, 4);
  }
  return colors;
};
...
var FlatColorPicker = function (colors, size) {
  ...
  for (var i = 0; i < 6; i++) {
    this.choices.push(
      new FlatSticker(size, 0.25 * size, 0.25 * size + 1.29 * lefts[i] * size, colors[i])
    );
...
RubiksCubeControls.prototype.setWidth = function (width) {
  ...
  for (var name in this.buttons) {
    ...
    this.buttons[name].style.left =
      (buttonWidth * 2) / 7 + (9 / 7) * buttonWidth * (current < 6 ? current : current - 6) + 'px';
